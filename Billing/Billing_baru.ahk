...
Daftar Hotkey:
----------------------------
Ctrl+Alt+A : Tambah koordinat (simpan posisi mouse saat ini ke slot)
Ctrl+Alt+D : Hapus koordinat dari slot
Ctrl+Alt+U : Gunakan koordinat (pindah mouse ke slot + klik kanan)
...
^!a::
    MouseGetPos, x, y
    InputBox, slot, Tambah Koordinat, Masukkan nomor slot (1-20):, , 200, 150
    if ErrorLevel
        return
    ; Validasi slot adalah angka
    if slot is not digit
    {
        MsgBox, Nomor slot harus angka.
        return
    }
    ; Simpan ke INI (gunakan backtick sebelum koma)
    IniWrite, %x%`,%y%, %IniFile%, Slots, %slot%
    MsgBox, Koordinat (%x%, %y%) disimpan untuk slot %slot%.
return

; Hotkey untuk menghapus koordinat (Ctrl+Alt+D)
^!d::
    InputBox, slot, Hapus Koordinat, Masukkan nomor slot yang akan dihapus:, , 200, 150
    if ErrorLevel
        return
    if slot is not digit
    {
    ...
    IniDelete, %IniFile%, Slots, %slot%
    MsgBox, Slot %slot% telah dihapus.
return

; Hotkey untuk menggunakan koordinat (Ctrl+Alt+U)
^!u::
    InputBox, slot, Gunakan Koordinat, Masukkan nomor slot:, , 200, 150
    if ErrorLevel
        return
    if slot is not digit
    {
    ...
    IniRead, koord, %IniFile%, Slots, %slot%, NOTFOUND
    if (koord = "NOTFOUND")
    {
        MsgBox, Slot %slot% tidak ditemukan.
        return
    }
