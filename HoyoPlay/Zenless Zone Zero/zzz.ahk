...
loop {
    if WinExist("ahk_exe HYP.exe") {
        ...
        if scan.Image("zzz.png", 0, &x, &y) or scan.Image("zzz1.png", 0, &x, &y) {
            Sleep 5000
            Click x, y
            break
        } else {
...
loop {
    if WinExist("ahk_exe ZenlessZoneZero.exe") {
        Sleep 1000
        WinActivate
        break
    }
...
#HotIf WinExist("ahk_class UnityWndClass") and WinExist("ahk_exe ZenlessZoneZero.exe") 

F1:: {
    Click 1280, 720
}
F2:: {
    ProcessClose("ZenlessZoneZero.exe")
